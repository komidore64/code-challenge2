...
OMG over 1900 lines of code! This was a tough problem but I _extremely_
...
```
$ git ls-files | grep -v LICENSE | xargs wc -l
...
`bin/code-challenge2` to start the ~~chaos~~magic!

help output:

```
$ bin/code-challenge2 --help
USAGE: code-challenge2 [OPTIONS]
...
6. Output results into a `output.json` file.
...
it has any Resources in their possession. If any Processor answers 'yes',
then the Director lets processing continue.

If the Director finds that all Resources have been processed, it will
instruct the workers to terminate, generate the `output.json` file, and
then exit.
...
The only code I changed was to tell the processor to wait a little
bit before asking the ResourcePool again for a resource when it was just handed
nil. This time I ran with all default settings and I didn't lose a resource!
